	...
	({ icon, title, content, link, image, explanation }) => {
		...
		return (
			...
			>
				...
				{explanation && (
					<div
						className={`absolute z-50 left-1/2 max-w-sm w-full sm:w-72 p-4 border border-1 border-zinc-500-bottom-6
			bg-gradient-to-br from-gray-50/98 to-gray-200/95
			dark:from-slate-700/95 dark:to-gray-800/95
			text-sm sm:text-base rounded-lg shadow-xl transform -translate-x-1/2 
			${
				isHovered
			...
			transition-all duration-300 ease-in-out`}
					...
					>
						<div className="relative">
							...
							<div
								className="absolute border border-1 border-zinc-500-bottom-6 right-1/2 sm:right-8 transform -translate-x-1/2 w-4 sm:w-6 h-4 sm:h-6 rotate-45
